...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `newclaw-e2e-${name}-`));
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        NEWCLAW_GATEWAY_TOKEN: inst.gatewayToken,
        NEWCLAW_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          NEWCLAW_GATEWAY_PORT: String(gwA.port),
          NEWCLAW_GATEWAY_TOKEN: gwA.gatewayToken,
          NEWCLAW_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          NEWCLAW_GATEWAY_PORT: String(gwB.port),
          NEWCLAW_GATEWAY_TOKEN: gwB.gatewayToken,
          NEWCLAW_GATEWAY_PASSWORD: "",
        }),
