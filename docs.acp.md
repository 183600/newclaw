...
This document describes how the NewClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`newclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
NewClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a NewClaw Gateway session.
...
- ACP client spawns `newclaw acp` and speaks ACP messages over stdio.
...
`newclaw acp` resolves the Gateway URL and auth from CLI flags or config:
