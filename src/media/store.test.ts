...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "NEWCLAW_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.NEWCLAW_STATE_DIR = path.join(home, ".newclaw");
    ...
    await fs.mkdir(path.join(home, ".newclaw"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.newclaw${path.sep}media`);
