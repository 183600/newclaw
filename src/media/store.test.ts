...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "IFLOW_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    ...
    await fs.mkdir(path.join(home, ".iflow"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.iflow${path.sep}media`);
