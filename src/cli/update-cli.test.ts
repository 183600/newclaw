...
vi.mock("../infra/newclaw-root.js", () => ({
  resolveNewClawPackageRoot: vi.fn(),
}));
...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveNewClawPackageRoot } = await import("../infra/newclaw-root.js");
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../process/exec.js");
    vi.mocked(resolveNewClawPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("NewClaw update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-update-"));
    try {
      await fs.writeFile(
      ...
      vi.mocked(resolveNewClawPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-update-wizard-"));
    const previousGitDir = process.env.NEWCLAW_GIT_DIR;
    try {
      setTty(true);
      process.env.NEWCLAW_GIT_DIR = tempDir;
      ...
      process.env.NEWCLAW_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
