...
function applySlotSelectionForPlugin(
  config: iFlowConfig,
  pluginId: string,
): { config: iFlowConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.iflow.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.iflow.ai/plugin");
      lines.push("");
