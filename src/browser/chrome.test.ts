...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    const userDataDir = await fsp.mkdtemp(path.join(os.tmpdir(), "newclaw-chrome-test-"));
    try {
      decorateNewClawProfile(userDataDir, { color: DEFAULT_NEWCLAW_BROWSER_COLOR });
      ...
      expect(def.name).toBe(DEFAULT_NEWCLAW_BROWSER_PROFILE_NAME);
      expect(def.shortcut_name).toBe(DEFAULT_NEWCLAW_BROWSER_PROFILE_NAME);
      ...
      const marker = await fsp.readFile(
        path.join(userDataDir, ".newclaw-profile-decorated"),
        "utf-8",
      );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await fsp.mkdtemp(path.join(os.tmpdir(), "newclaw-chrome-test-"));
    try {
      decorateNewClawProfile(userDataDir, { color: "lobster-orange" });
      const localState = await readJson(path.join(userDataDir, "Local State"));
      const profile = localState.profile as Record<string, unknown>;
      const infoCache = profile.info_cache as Record<string, unknown>;
      const def = infoCache.Default as Record<string, unknown>;
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    try {
      ...
      expect(profile.name).toBe(DEFAULT_NEWCLAW_BROWSER_PROFILE_NAME);
    } finally {
      await fsp.rm(userDataDir, { recursive: true, force: true });
    }
  });
...
describe("browser chrome helpers", () => {
  ...
  it("stopNewClawChrome no-ops when process is already killed", async () => {
    const proc = { killed: true, exitCode: null, kill: vi.fn() };
    await stopNewClawChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopNewClawChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopNewClawChrome sends SIGTERM and returns once CDP is down", async () => {
