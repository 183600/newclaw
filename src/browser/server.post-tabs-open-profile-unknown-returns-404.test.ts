...
vi.mock("./chrome.js", () => ({
  ...
  launchClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
describe("profile CRUD endpoints", () => {
  ...
  it("POST /profiles/create returns 409 for duplicate name", async () => {
    ...
    // "claw" already exists as the default profile
    const result = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "claw" }),
    });
  ...
  it("DELETE /profiles/:name returns 400 for default profile deletion", async () => {
    ...
    // claw is the default profile
    const result = await realFetch(`${base}/profiles/claw`, {
      method: "DELETE",
    });
    expect(result.status).toBe(400);
    const body = (await result.json()) as { error: string };
