import type { BrowserConfig, BrowserProfileConfig, iFlowConfig } from "../config/config.js";
...
import {
  DEFAULT_IFLOW_BROWSER_COLOR,
  DEFAULT_IFLOW_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_IFLOW_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_IFLOW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "iflow" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_IFLOW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_IFLOW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an iFlow browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for iflow/iflow CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: iFlowConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_IFLOW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "iflow";
