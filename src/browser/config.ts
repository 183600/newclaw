import type { BrowserConfig, BrowserProfileConfig, NewClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_NEWCLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_NEWCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "newclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_NEWCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_NEWCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an NewClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for newclaw/newclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: NewClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_NEWCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "newclaw";
