...
vi.mock("./chrome.js", () => ({
  resolveClawUserDataDir: vi.fn(() => "/tmp/claw-test/claw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "claw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveClawUserDataDir).mockReturnValue(userDataDir);
