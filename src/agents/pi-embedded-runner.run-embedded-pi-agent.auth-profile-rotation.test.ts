...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): NewClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-workspace-"));
    try {
