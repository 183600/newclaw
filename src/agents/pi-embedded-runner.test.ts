...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-embedded-agent-"));
...
const ensureModels = (cfg: NewClawConfig) => ensureNewClawModelsJson(cfg, agentDir) as unknown;
