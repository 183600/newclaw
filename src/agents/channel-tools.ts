...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as iFlowConfig);
...
export function listChannelAgentTools(params: { cfg?: iFlowConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: iFlowConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
