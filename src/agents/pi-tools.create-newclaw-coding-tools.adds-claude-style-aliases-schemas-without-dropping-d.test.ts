...
import { createNewClawCodingTools } from "./pi-tools.js";

const defaultTools = createNewClawCodingTools();

describe("createNewClawCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createNewClawCodingTools();
    ...
    try {
      ...
      const contents = "Hello from newclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "newclaw-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "newclaw-workspace"),
      workspaceAccess: "none",
      containerName: "newclaw-sbx-test",
    ...
    const tools = createNewClawCodingTools({ sandbox });
