...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.NEWCLAW_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-test-"));
    process.env.NEWCLAW_STATE_DIR = stateDir;
    process.env.NEWCLAW_PROFILE = "isolated";

    try {
      const tool = createNewClawTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: newclaw --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/newclaw" } }\n}\n';
