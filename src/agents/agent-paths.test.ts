...
describe("resolveNewClawAgentDir", () => {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-agent-"));
    process.env.NEWCLAW_STATE_DIR = tempStateDir;
    delete process.env.NEWCLAW_AGENT_DIR;
    delete process.env.PI_CODING_AGENT_DIR;

    const resolved = resolveNewClawAgentDir();

    expect(resolved).toBe(path.join(tempStateDir, "agents", "main", "agent"));
  });

  it("honors NEWCLAW_AGENT_DIR overrides", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-agent-"));
    const override = path.join(tempStateDir, "agent");
    process.env.NEWCLAW_AGENT_DIR = override;
    delete process.env.PI_CODING_AGENT_DIR;
