...
/**
 * Cleanup suspended NewClaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: NewClawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-cli-images-"));
  const paths: string[] = [];
