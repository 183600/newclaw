...
import { createNewClawTools } from "./newclaw-tools.js";
import { __testing, createNewClawCodingTools } from "./pi-tools.js";
...
describe("createNewClawCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createNewClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createNewClawCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "newclaw-outside.txt");
