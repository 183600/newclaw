...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.IFLOW_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "iflow-test-"));
    ...
    try {
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: iflow --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/iflow" } }\n}\n';
