...
function pickFirstExistingAgentId(cfg: NewClawConfig, agentId: string): string {
  ...
  // Return DEFAULT_AGENT_ID only when the agent list is not empty and the agent is not found
...
function matchesChannel(
  ...
  return key === normalizeToken(channel);
}
...
function matchesPeer(
  ...
  const peerKind = normalizeToken(peer.kind);
  const peerId = normalizeToken(peer.id);
  if (!kind || !id) {
    return false;
  }
  const result = kind === peerKind && id === peerId;
  return result;
}
...
export function resolveAgentRoute(input: ResolveAgentRouteInput): ResolvedAgentRoute {
  ...
  const bindings = listBindings(input.cfg).filter((binding) => {
    ...
    if (!binding.agentId || typeof binding.agentId !== "string" || !binding.agentId.trim()) {
      return false;
    }
