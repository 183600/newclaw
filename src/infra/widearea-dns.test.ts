...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expect(txt).toContain(`$ORIGIN iflow.internal.`);
    ...
    expect(txt).toContain(`_iflow-gw._tcp IN PTR studio-london._iflow-gw._tcp`);
    expect(txt).toContain(`studio-london._iflow-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (iFlow)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/iflow`);
  });
