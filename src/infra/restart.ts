...
export function triggeriFlowRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(process.env.IFLOW_SYSTEMD_UNIT, process.env.IFLOW_PROFILE);
  ...
  const label =
    process.env.IFLOW_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.IFLOW_PROFILE);
