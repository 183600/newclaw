...
export function triggerNewClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.NEWCLAW_SYSTEMD_UNIT,
        process.env.NEWCLAW_PROFILE,
      );
  ...
  const label =
    process.env.NEWCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.NEWCLAW_PROFILE);
