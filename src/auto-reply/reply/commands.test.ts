...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "iflow-commands-"));
...
function buildParams(commandBody: string, cfg: iFlowConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
