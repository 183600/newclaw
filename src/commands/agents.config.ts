...
export function listAgentEntries(cfg: iFlowConfig): AgentEntry[] {
...
function resolveAgentName(cfg: iFlowConfig, agentId: string) {
...
function resolveAgentModel(cfg: iFlowConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: iFlowConfig): AgentSummary[] {
