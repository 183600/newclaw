...
export function listAgentEntries(cfg: NewClawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: NewClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: NewClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: NewClawConfig): AgentSummary[] {
