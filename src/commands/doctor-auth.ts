...
function pruneAuthProfiles(
  cfg: NewClawConfig,
  profileIds: Set<string>,
): { next: NewClawConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("newclaw models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("newclaw models auth setup-token")} or ${formatCliCommand(
      "newclaw configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "newclaw models auth login --provider openai-codex",
    )} or ${formatCliCommand("newclaw configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("newclaw configure")}\` or \`${formatCliCommand("newclaw onboard")}\`.`;
}
