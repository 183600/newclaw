...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.IFLOW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
vi.mock("../plugins/loader.js", () => ({
  loadiFlowPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  ...
  it("does not warn when only the active workspace is present", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      ...
      valid: true,
      config: {
        agents: { defaults: { workspace: "/Users/steipete/iflow" } },
      },
      issues: [],
      legacyIssues: [],
    });
    ...
    const legacyPath = path.join("/Users/steipete", "iflow");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (value === "/Users/steipete/iflow" || value === legacyPath || value === legacyAgentsPath) {
        return true;
      }
