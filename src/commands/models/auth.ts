...
import { readConfigFileSnapshot, type iFlowConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: iFlowConfig, model: string): iFlowConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("iflow plugins install")}\`.`,
    );
  }
