...
import type { iFlowConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: iFlowConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: iFlowConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as iFlowConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as iFlowConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as iFlowConfig)),
    ),
  };
}
