...
import type { NewClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: NewClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: NewClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = NewClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as NewClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as NewClawConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as NewClawConfig)),
    ),
  };
}
