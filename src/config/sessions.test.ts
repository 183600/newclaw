...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-newclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-newclaw#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-sessions-"));
  ...
  it("derives session transcripts dir from NEWCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { NEWCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
