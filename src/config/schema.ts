...
export type ConfigSchema = ReturnType<typeof ClawSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when Claw writes the config.",
  ...
  "gateway.controlUi.basePath": "Optional URL prefix where the Control UI is served (e.g. /claw).",
  ...
  "memory.backend": 'Memory backend ("builtin" for Claw embeddings, "qmd" for QMD sidecar).',
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `claw plugins update` to locate install sources).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/claw",
  "channels.mattermost.baseUrl": "https://chat.example.com",
  "agents.list[].identity.avatar": "avatars/claw.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = ClawSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
