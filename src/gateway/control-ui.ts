...
function injectControlUiConfig(html: string, opts: ControlUiInjectionOpts): string {
  ...
  const script =
    `<script>` +
    `window.__NEWCLAW_CONTROL_UI_BASE_PATH__=${JSON.stringify(basePath)};` +
    `window.__NEWCLAW_ASSISTANT_NAME__=${JSON.stringify(
      assistantName ?? DEFAULT_ASSISTANT_IDENTITY.name,
    )};` +
    `window.__NEWCLAW_ASSISTANT_AVATAR__=${JSON.stringify(
      assistantAvatar ?? DEFAULT_ASSISTANT_IDENTITY.avatar,
    )};` +
    `</script>`;
  // Check if already injected
  if (html.includes("__NEWCLAW_ASSISTANT_NAME__")) {
    return html;
  }
