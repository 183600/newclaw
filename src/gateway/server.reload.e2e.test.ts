...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.IFLOW_SKIP_CHANNELS;
    prevSkipGmail = process.env.IFLOW_SKIP_GMAIL_WATCHER;
    process.env.IFLOW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.IFLOW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.IFLOW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.IFLOW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
