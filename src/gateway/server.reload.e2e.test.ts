...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.NEWCLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.NEWCLAW_SKIP_GMAIL_WATCHER;
    process.env.NEWCLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.NEWCLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.NEWCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.NEWCLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
