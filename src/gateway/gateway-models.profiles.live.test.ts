...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveiFlowAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "iflow-live-state-"));
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "iFlow live tool probe (local, safe): " +
