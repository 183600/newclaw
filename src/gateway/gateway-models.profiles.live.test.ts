...
import type { iFlowConfig, ModelProviderConfig } from "../config/types.js";
...
import { ensureClawModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.IFLOW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.IFLOW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.IFLOW_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<iFlowConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.IFLOW_AGENT_DIR,
  ...
  const hostAgentDir = resolveClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "claw-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.iflow-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveClawAgentDir();
  const sanitizedCfg: iFlowConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "claw-live-"));
  const tempConfigPath = path.join(tempDir, "iflow.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.IFLOW_CONFIG_PATH = tempConfigPath;

  await ensureClawModelsJson(nextCfg);
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Claw live tool probe (local, safe): " +
    ...
    process.env.IFLOW_CONFIG_PATH = previous.configPath;
    process.env.IFLOW_GATEWAY_TOKEN = previous.token;
    process.env.IFLOW_SKIP_CHANNELS = previous.skipChannels;
    process.env.IFLOW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.IFLOW_SKIP_CRON = previous.skipCron;
    process.env.IFLOW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.IFLOW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.IFLOW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.IFLOW_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.iflow-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
