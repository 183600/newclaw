...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.IFLOW_STATE_DIR;
  previousConfigPath = process.env.IFLOW_CONFIG_PATH;
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.IFLOW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.IFLOW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.IFLOW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.IFLOW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.IFLOW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.IFLOW_SKIP_CANVAS_HOST;
    } else {
      process.env.IFLOW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.IFLOW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.IFLOW_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.IFLOW_GATEWAY_PASSWORD;
