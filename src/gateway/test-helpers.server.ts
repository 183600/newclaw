...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.NEWCLAW_STATE_DIR;
  previousConfigPath = process.env.NEWCLAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.NEWCLAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.NEWCLAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.NEWCLAW_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-gateway-home-"));
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "newclaw-test-"))
    : path.join(tempHome, ".newclaw-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.NEWCLAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.NEWCLAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.NEWCLAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.NEWCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.NEWCLAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.NEWCLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.NEWCLAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.NEWCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.NEWCLAW_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.NEWCLAW_GATEWAY_PASSWORD;
