...
const execHostEnforced = process.env.NEWCLAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.NEWCLAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureNewClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
