...
const execHostEnforced = process.env.IFLOW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.IFLOW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureiFlowCliOnPath({ pathEnv: process.env.PATH ?? "" });
