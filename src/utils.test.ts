...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "newclaw-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "newclaw-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "newclaw-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "newclaw-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.newclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "newclaw-config-dir-"));
    try {
      const newDir = path.join(root, ".newclaw");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/newclaw")).toBe(path.resolve(os.homedir(), "newclaw"));
  });
