...
Not directly. macOS skills are gated by `metadata.iflow.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `imsg`, `apple-notes`, `apple-reminders`) will not load unless you override the gating.
...
- The Control UI stores the token in browser localStorage key `iflow.control.settings.v1`.
