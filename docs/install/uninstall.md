...
- **Easy path** if `newclaw` is still installed.
...
Use this if the gateway service keeps running but `newclaw` is missing.

### macOS (launchd)

Default label is `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.newclaw.*` may still exist):
...
If you used a profile, replace the label and plist name with `bot.molt.<profile>`. Remove any legacy `com.newclaw.*` plists if present.
...
If you used a profile, delete the matching task name and `~\.newclaw-<profile>\gateway.cmd`.

## Normal install vs source checkout

### Normal install (install.sh / npm / pnpm / bun)

If you used `https://newclaw.ai/install.sh` or `install.ps1`, the CLI was installed with `npm install -g newclaw@latest`.
Remove it with `npm rm -g newclaw` (or `pnpm remove -g` / `bun remove -g` if you installed that way).
