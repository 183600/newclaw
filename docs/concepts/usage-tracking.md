...
- `/usage cost` in chats: local cost summary aggregated from iFlow session logs.
- CLI: `iflow status --usage` prints a full per-provider breakdown.
- CLI: `iflow channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
