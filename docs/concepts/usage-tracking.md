...
- `/usage cost` in chats: local cost summary aggregated from NewClaw session logs.
- CLI: `newclaw status --usage` prints a full per-provider breakdown.
- CLI: `newclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
