...
Deploy iFlow on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/iflow` — Control UI
...
- `IFLOW_STATE_DIR=/data/.iflow` (recommended)
- `IFLOW_WORKSPACE_DIR=/data/workspace` (recommended)
- `IFLOW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your iFlow state + workspace so you can migrate to another host without losing config or memory.
