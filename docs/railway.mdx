...
Deploy NewClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/newclaw` — Control UI
...
- `NEWCLAW_STATE_DIR=/data/.newclaw` (recommended)
- `NEWCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `NEWCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your NewClaw state + workspace so you can migrate to another host without losing config or memory.
