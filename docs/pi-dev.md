...
This guide summarizes a sane workflow for working on the pi integration in iFlow.
...
- Trigger the agent directly:
  - `pnpm iflow agent --message "Hello" --thinking low`
...
State lives under the iFlow state directory. Default is `~/.iflow`. If `IFLOW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `iflow.json` for config
...
- https://docs.iflow.ai/testing
- https://docs.iflow.ai/start/getting-started
