...
This guide summarizes a sane workflow for working on the pi integration in NewClaw.
...
- Trigger the agent directly:
  - `pnpm newclaw agent --message "Hello" --thinking low`
...
State lives under the NewClaw state directory. Default is `~/.newclaw`. If `NEWCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `newclaw.json` for config
...
- https://docs.newclaw.ai/testing
- https://docs.newclaw.ai/start/getting-started
