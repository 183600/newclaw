...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/newclaw`)
...
{
  gateway: {
    controlUi: { enabled: true, basePath: "/newclaw" }, // basePath optional
  },
}
```
...
```json5
{
  gateway: {
    ...
    auth: { mode: "password" }, // or NEWCLAW_GATEWAY_PASSWORD
  },
}
```
