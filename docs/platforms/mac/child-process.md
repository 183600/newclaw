...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`IFLOW_PROFILE`; legacy `com.iflow.*` is supported).
...
- Writes `~/.iflow/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.iflow/disable-launchagent`,
