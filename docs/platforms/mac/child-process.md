...
service via the external `newclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`NEWCLAW_PROFILE`; legacy `com.newclaw.*` is supported).
...
- Writes `~/.newclaw/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.newclaw/disable-launchagent`,
