...
- sets a stable debug bundle identifier: `ai.iflow.mac.debug`
...
- inject build metadata into Info.plist: `ClawBuildTimestamp` (UTC) and `ClawGitCommit` (short hash) so the About pane can show build, git, and debug/release channel.
...
- `ClawBuildTimestamp`: ISO8601 UTC at package time
- `ClawGitCommit`: short git hash (or `unknown` if unavailable)
