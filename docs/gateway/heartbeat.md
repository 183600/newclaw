...
- During heartbeat runs, NewClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `newclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, NewClaw skips the heartbeat run entirely (no model call).
