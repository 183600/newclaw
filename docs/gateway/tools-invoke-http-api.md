...
NewClaw’s Gateway exposes a simple HTTP endpoint for invoking a single tool directly. It is always enabled, but gated by Gateway auth and tool policy.
...
- `x-newclaw-message-channel: <channel>` (example: `slack`, `telegram`)
- `x-newclaw-account-id: <accountId>` (when multiple accounts exist)
