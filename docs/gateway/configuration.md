...
```json5
{
  agents: {
    ...
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@iflow", "reisponde"] },
      },
    ],
  },
  channels: {
...
{ "$include": "/etc/iflow/base.json5" } // absolute
...
- Default log file: `/tmp/newclaw/iflow-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/newclaw/iflow.log`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@iflow", "iflow"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@iflow"],
        },
      },
    ],
  },
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["iflow-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/iflow" } },
}
```
...
`[iflow]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[iflow]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[iflow]"`, NewClaw will instead use `[{identity.name}]` when the routed
...
- `group:nodes`: `nodes`
- `group:iflow`: all built-in NewClaw tools (excludes provider plugins)
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          containerPrefix: "iflow-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
from `~/.newclaw/extensions`, `<workspace>/.iflow/extensions`, plus any
...
- Hostname: defaults to `iflow` (advertises `newclaw.local`). Override with `NEWCLAW_MDNS_HOSTNAME`.
