---
summary: "All configuration options for ~/.newclaw/newclaw.json with examples"
read_when:
...
NewClaw reads an optional **JSON5** config from `~/.newclaw/newclaw.json` (comments + trailing commas allowed).

If the file is missing, NewClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.newclaw/workspace`). You usually only need a config to:
...
NewClaw only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `newclaw doctor`, `newclaw logs`, `newclaw health`, `newclaw status`, `newclaw service`, `newclaw help`).
- Run `newclaw doctor` to see the exact issues.
- Run `newclaw doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `newclaw config set`. Keep a backup of `~/.newclaw/newclaw.json`.
...
```bash
newclaw gateway call config.get --params '{}' # capture payload.hash
newclaw gateway call config.apply --params '{
...
```bash
newclaw gateway call config.get --params '{}' # capture payload.hash
newclaw gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.newclaw/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@newclaw", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.newclaw/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.newclaw/workspace" }],
}
```
...
{ "$include": "/etc/newclaw/base.json5" } // absolute
...
```json5
// ~/.newclaw/clients/mueller/agents.json5
[
...
```json5
// ~/.newclaw/clients/mueller/broadcast.json5
{
...
NewClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, NewClaw runs your login shell and imports only the missing expected keys (never overrides).
...
- `NEWCLAW_LOAD_SHELL_ENV=1`
- `NEWCLAW_SHELL_ENV_TIMEOUT_MS=15000`
...
NewClaw stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.newclaw/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.newclaw/credentials/oauth.json` (or `$NEWCLAW_STATE_DIR/credentials/oauth.json`)
...
- `~/.newclaw/agent/*` (migrated by `newclaw doctor` into `~/.newclaw/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `NEWCLAW_OAUTH_DIR`
- Agent dir (default agent root override): `NEWCLAW_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, NewClaw imports `oauth.json` entries into `auth-profiles.json`.
...
If set, NewClaw derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/newclaw/newclaw-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/newclaw/newclaw.log`.
...
- The legacy single-account Baileys auth dir is migrated by `newclaw doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@newclaw", "newclaw"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@newclaw"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.newclaw/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.newclaw/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.newclaw/workspace-home" },
      { id: "work", workspace: "~/.newclaw/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `newclaw.json`).
...
NewClaw starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["newclaw-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
NewClaw starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `newclaw plugins install @newclaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
NewClaw starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
NewClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, NewClaw
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/newclaw" } },
}
```
...
When a file exceeds this limit, NewClaw logs a warning and injects a truncated
...
message envelopes). If unset, NewClaw uses the host timezone at runtime.
...
`[newclaw]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[newclaw]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[newclaw]"`, NewClaw will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, NewClaw generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.newclaw/settings/tts.json",
      elevenlabs: {
...
NewClaw also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, NewClaw currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `newclaw configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:newclaw`: all built-in NewClaw tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.newclaw/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "newclaw-sandbox-browser:bookworm-slim",
          containerPrefix: "newclaw-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
NewClaw uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.newclaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
NewClaw config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, NewClaw writes/merges a `models.json` into
`~/.newclaw/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. NewClaw uses
...
- Shortcut: `newclaw onboard --auth-choice opencode-zen`.
...
Shortcut: `newclaw onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `newclaw onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `newclaw onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `newclaw onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `newclaw onboard --auth-choice minimax-api`.
...
- Override the agent config root with `NEWCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.newclaw/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.newclaw/agents/<agentId>/sessions/sessions.json
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, NewClaw stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.newclaw/skills` (workspace skills
...
from `~/.newclaw/extensions`, `<workspace>/.newclaw/extensions`, plus any
...
### `browser` (newclaw-managed browser)

NewClaw can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for newclaw and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (NewClaw.app menubar, or `newclaw gateway`).
...
- Examples: `"/ui"`, `"/newclaw"`, `"/apps/newclaw"`.
...
- When a connection comes from one of these IPs, NewClaw uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `newclaw gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `NEWCLAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `NEWCLAW_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. NewClaw
...
- NewClaw.app watches `~/.newclaw/newclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.newclaw/newclaw.json` (or `NEWCLAW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.newclaw/newclaw.json` (or `NEWCLAW_CONFIG_PATH`)
...
- `NEWCLAW_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `newclaw --dev …` → uses `~/.newclaw-dev` + shifts ports from base `19001`
- `newclaw --profile <name> …` → uses `~/.newclaw-<name>` (port via config/env/flags)
...
```bash
NEWCLAW_CONFIG_PATH=~/.newclaw/a.json \
NEWCLAW_STATE_DIR=~/.newclaw-a \
...
Gmail helper config (used by `newclaw webhooks gmail setup` / `run`):
...
- Set `NEWCLAW_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, NewClaw defaults `serve.path` to `/` so
...
Default root: `~/.newclaw/workspace/canvas`  
Default port: `18793` (chosen to avoid the newclaw browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__newclaw__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__newclaw__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `NEWCLAW_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.newclaw/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.newclaw/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.newclaw/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_newclaw-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `newclaw` (advertises `newclaw.local`). Override with `NEWCLAW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_newclaw-gw._tcp` under `~/.newclaw/dns/` using the configured discovery domain (example: `newclaw.internal.`).
