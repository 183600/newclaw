---
summary: "All configuration options for ~/.iflow/iflow.json with examples"
read_when:
...
iFlow reads an optional **JSON5** config from `~/.iflow/iflow.json` (comments + trailing commas allowed).

If the file is missing, iFlow uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.iflow/workspace`). You usually only need a config to:
...
iFlow only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `iflow doctor`, `iflow logs`, `iflow health`, `iflow status`, `iflow service`, `iflow help`).
- Run `iflow doctor` to see the exact issues.
- Run `iflow doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `iflow config set`. Keep a backup of `~/.iflow/iflow.json`.
...
```bash
iflow gateway call config.get --params '{}' # capture payload.hash
iflow gateway call config.apply --params '{
...
```bash
iflow gateway call config.get --params '{}' # capture payload.hash
iflow gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.iflow/workspace" },
    list: [
      {
        id: "main",
...
// ~/.iflow/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.iflow/workspace" }],
}
```
...
```json5
// ~/.iflow/clients/mueller/agents.json5
[
...
```json5
// ~/.iflow/clients/mueller/broadcast.json5
{
...
iFlow reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, iFlow runs your login shell and imports only the missing expected keys (never overrides).
...
- `IFLOW_LOAD_SHELL_ENV=1`
- `IFLOW_SHELL_ENV_TIMEOUT_MS=15000`
...
iFlow stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.iflow/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.iflow/credentials/oauth.json` (or `$IFLOW_STATE_DIR/credentials/oauth.json`)
...
- `~/.iflow/agent/*` (migrated by `iflow doctor` into `~/.iflow/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `IFLOW_OAUTH_DIR`
- Agent dir (default agent root override): `IFLOW_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, iFlow imports `oauth.json` entries into `auth-profiles.json`.
...
If set, iFlow derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/iflow/iflow-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/iflow/iflow.log`.
...
- The legacy single-account Baileys auth dir is migrated by `iflow doctor` into `whatsapp/default`.
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.iflow/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.iflow/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.iflow/workspace-home" },
      { id: "work", workspace: "~/.iflow/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `iflow.json`).
...
iFlow starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
iFlow starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `iflow plugins install @iflow/mattermost` (or `./extensions/mattermost` from a git checkout).
...
iFlow starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
iFlow spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, iFlow
...
When a file exceeds this limit, iFlow logs a warning and injects a truncated
...
message envelopes). If unset, iFlow uses the host timezone at runtime.
...
`"[iflow]"`, iFlow will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, iFlow generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.iflow/settings/tts.json",
      elevenlabs: {
...
iFlow also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, iFlow currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `iflow configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:iflow`: all built-in iFlow tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.iflow/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "iflow-sandbox-browser:bookworm-slim",
...
iFlow uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.iflow/agents/<agentId>/agent/models.json` or by defining the same schema inside your
iFlow config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, iFlow writes/merges a `models.json` into
`~/.iflow/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. iFlow uses
...
- Shortcut: `iflow onboard --auth-choice opencode-zen`.
...
Shortcut: `iflow onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `iflow onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `iflow onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `iflow onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `iflow onboard --auth-choice minimax-api`.
...
- Override the agent config root with `IFLOW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.iflow/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.iflow/agents/<agentId>/sessions/sessions.json
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, iFlow stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.iflow/skills` (workspace skills
...
from `~/.iflow/extensions`, `<workspace>/.iflow/extensions`, plus any
...
iFlow can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for iflow and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (iFlow.app menubar, or `iflow gateway`).
...
- When a connection comes from one of these IPs, iFlow uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `iflow gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `IFLOW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `IFLOW_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. iFlow
...
- iFlow.app watches `~/.iflow/iflow.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.iflow/iflow.json` (or `IFLOW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.iflow/iflow.json` (or `IFLOW_CONFIG_PATH`)
...
- `IFLOW_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `iflow --dev …` → uses `~/.iflow-dev` + shifts ports from base `19001`
- `iflow --profile <name> …` → uses `~/.iflow-<name>` (port via config/env/flags)
...
```bash
IFLOW_CONFIG_PATH=~/.iflow/a.json \
IFLOW_STATE_DIR=~/.iflow-a \
...
Gmail helper config (used by `iflow webhooks gmail setup` / `run`):
...
- Set `IFLOW_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, iFlow defaults `serve.path` to `/` so
...
Default root: `~/.iflow/workspace/canvas`  
Default port: `18793` (chosen to avoid the iflow browser CDP port `18792`)  
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.iflow/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.iflow/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.iflow/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_iflow-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `iflow` (advertises `iflow.local`). Override with `IFLOW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_iflow-gw._tcp` under `~/.iflow/dns/` using the configured discovery domain (example: `iflow.internal.`).
