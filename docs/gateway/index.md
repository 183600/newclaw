...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__claw__/canvas/` from `~/.newclaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `NEWCLAW_SKIP_CANVAS_HOST=1`.
