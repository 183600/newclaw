---
summary: "Chrome extension: let NewClaw drive your existing Chrome tab"
read_when:
...
The NewClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate newclaw-managed Chrome profile.
...
NewClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the NewClaw release (npm package) as static files. There is no separate “build” step.

After upgrading NewClaw:

- Re-run `newclaw browser extension install` to refresh the installed files under your NewClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

NewClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want NewClaw to control.
...
- `ON`: attached; NewClaw can drive that tab.
...
Debugging: `newclaw sandbox explain`
...
`newclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `newclaw browser extension install` first to copy the extension to a stable location under your NewClaw state directory.
...
- **This is not isolated** like the dedicated newclaw-managed profile.
