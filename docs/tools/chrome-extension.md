---
summary: "Chrome extension: let iFlow drive your existing Chrome tab"
read_when:
...
The iFlow Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate iflow-managed Chrome profile.
...
iFlow then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the iFlow release (npm package) as static files. There is no separate “build” step.

After upgrading iFlow:

- Re-run `iflow browser extension install` to refresh the installed files under your iFlow state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

iFlow ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want iFlow to control.
...
- `ON`: attached; iFlow can drive that tab.
...
Debugging: `iflow sandbox explain`
...
`iflow browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `iflow browser extension install` first to copy the extension to a stable location under your iFlow state directory.
