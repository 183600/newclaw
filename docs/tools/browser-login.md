...
When a site requires login, **sign in manually** in the **host** browser profile (the newclaw browser).
...
NewClaw controls a **dedicated Chrome profile** (named `newclaw`, orange‑tinted UI). This is separate from your daily browser profile.
...
If you have multiple profiles, pass `--browser-profile <name>` (the default is `newclaw`).
