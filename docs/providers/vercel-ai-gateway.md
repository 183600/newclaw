...
read_when:
  - You want to use Vercel AI Gateway with NewClaw
...
```bash
newclaw onboard --auth-choice ai-gateway-api-key
```
...
is available to that process (for example, in `~/.newclaw/.env` or via
`env.shellEnv`).
