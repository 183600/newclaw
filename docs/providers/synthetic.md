---
summary: "Use Synthetic's Anthropic-compatible API in iFlow"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. iFlow registers it as the
...
Note: iFlow's Anthropic client appends `/v1` to the base URL, so use
