---
summary: "Use Synthetic's Anthropic-compatible API in NewClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. NewClaw registers it as the
...
Note: NewClaw's Anthropic client appends `/v1` to the base URL, so use
