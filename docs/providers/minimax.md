---
summary: "Use MiniMax M2.1 in iFlow"
read_when:
  - You want MiniMax models in iFlow
...
```bash
iflow plugins enable minimax-portal-auth  # skip if already loaded.
iflow gateway restart  # restart if gateway is already running
iflow onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/iflow/iflow/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `iflow.json`:
...
- Use `iflow models list` and `iflow models set minimax/MiniMax-M2.1` to switch.
...
- Running `iflow configure` and selecting **MiniMax M2.1**, or
