---
summary: "Use MiniMax M2.1 in NewClaw"
read_when:
  - You want MiniMax models in NewClaw
...
```bash
newclaw plugins enable minimax-portal-auth  # skip if already loaded.
newclaw gateway restart  # restart if gateway is already running
newclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/newclaw/newclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `newclaw.json`:
...
- Use `newclaw models list` and `newclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `newclaw configure` and selecting **MiniMax M2.1**, or
