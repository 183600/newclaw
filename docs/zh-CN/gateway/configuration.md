...
{ "$include": "/etc/iflow/base.json5" } // 绝对路径
...
- 默认日志文件：`/tmp/newclaw/iflow-YYYY-MM-DD.log`
- 如需稳定路径，将 `logging.file` 设为 `/tmp/newclaw/iflow.log`。
- 控制台输出可通过以下方式单独调整：
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["iflow-dm"], // 可选群组私聊白名单
      },
      guilds: {
        "123456789012345678": {
          // 服务器 id（推荐）或 slug
...
如果未设置 `messages.responsePrefix`，默认不应用前缀。WhatsApp 自聊天回复是例外：它们在设置时默认为 `[{identity.name}]`，否则为 `[iflow]`，以保持同一手机上的对话可读性。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[iflow]"`，否则为 `""`（无前缀）。使用 `"[iflow]"` 时，如果路由的智能体设置了 `identity.name`，NewClaw 会改用 `[{identity.name}]`。
...
- `group:nodes`：`nodes`
- `group:iflow`：所有内置 NewClaw 工具（不包含提供商插件）
...
控制插件发现、允许/拒绝和每插件配置。插件从 `~/.newclaw/extensions`、`<workspace>/.iflow/extensions` 以及任何 `plugins.load.paths` 条目加载。**配置更改需要重启 Gateway网关。**
...
- 监视目录并通过 `/__claw__/ws` 的 WebSocket 端点广播重载
- 目录为空时自动创建起始 `index.html`（以便你立即看到内容）
- 同时在 `/__claw__/a2ui/` 提供 A2UI，并作为 `canvasHostUrl` 通告给节点
...
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `iflow`（通告 `newclaw.local`）。通过 `NEWCLAW_MDNS_HOSTNAME` 覆盖。
