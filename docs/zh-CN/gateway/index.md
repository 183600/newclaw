...
- 默认在 `canvasHost.port`（默认 `18793`）启动 Canvas 文件服务器，从 `~/.newclaw/workspace/canvas` 提供 `http://<gateway-host>:18793/__claw__/canvas/` 服务。通过 `canvasHost.enabled=false` 或 `NEWCLAW_SKIP_CANVAS_HOST=1` 禁用。
