...
每个插件的根目录中必须包含一个 `newclaw.plugin.json` 文件。如果路径指向一个文件，插件根目录就是该文件所在目录，且必须包含清单文件。

如果多个插件解析到相同的 id，按上述顺序第一个匹配的生效，较低优先级的副本将被忽略。

### 包集合

插件目录可以包含一个带有 `newclaw.extensions` 的 `package.json`：
...
渠道插件可以通过 `newclaw.channel` 公布新手引导元数据，并通过 `newclaw.install` 提供安装提示。这使核心目录保持无数据状态。
...
或者将 `NEWCLAW_PLUGIN_CATALOG_PATHS`（或 `NEWCLAW_MPM_CATALOG_PATHS`）指向一个或多个 JSON 文件（以逗号/分号/`PATH` 分隔）。每个文件应包含 `{ "entries": [ { "name": "@scope/pkg", "newclaw": { "channel": {...}, "install": {...} } } ] }`。
...
- 插件配置使用 `newclaw.plugin.json` 中嵌入的 JSON Schema（`configSchema`）进行验证。
...
- 插件 `package.json` 必须包含 `newclaw.extensions`，其中列出一个或多个入口文件。
...
- 单独发布的插件应运行自己的 CI（lint/构建/测试），并验证 `newclaw.extensions` 指向构建后的入口点（`dist/index.js`）。
