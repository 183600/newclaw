...
插件目录可以包含一个带有 `iflow.extensions` 的 `package.json`：
...
渠道插件可以通过 `iflow.channel` 公布新手引导元数据，并通过 `iflow.install` 提供安装提示。这使核心目录保持无数据状态。
...
- 插件配置使用 `iflow.plugin.json` 中嵌入的 JSON Schema（`configSchema`）进行验证。
...
- 单独发布的插件应运行自己的 CI（lint/构建/测试），并验证 `iflow.extensions` 指向构建后的入口点（`dist/index.js`）。
