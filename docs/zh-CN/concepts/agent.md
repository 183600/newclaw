...
iFlow 运行一个源自 **pi-mono** 的单一嵌入式智能体运行时。

## 工作区（必需）

iFlow 使用单一智能体工作区目录（`agents.defaults.workspace`）作为智能体工具和上下文的**唯一**工作目录（`cwd`）。

建议：使用 `iflow setup` 创建 `~/.iflow/iflow.json`（如果不存在）并初始化工作区文件。
...
在 `agents.defaults.workspace` 内，iFlow 期望这些用户可编辑的文件：
...
在新会话的第一轮对话中，iFlow 会将这些文件的内容直接注入到智能体上下文中。

空白文件会被跳过。大文件会被裁剪和截断并附带标记，以保持提示词精简（阅读完整文件以获取全部内容）。

如果文件缺失，iFlow 会注入一行"文件缺失"标记（`iflow setup` 会创建安全的默认模板）。
...
iFlow 从三个位置加载 Skills（名称冲突时工作区优先）：
...
iFlow 复用了 pi-mono 代码库的部分内容（模型/工具），但**会话管理、发现和工具连接由 iFlow 负责**。
...
- 如果省略提供商，iFlow 会将输入视为别名或**默认提供商**的模型（仅在模型 ID 中没有 `/` 时有效）。
