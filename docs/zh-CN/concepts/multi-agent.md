...
- **会话存储**（聊天历史 + 路由状态），位于 `~/.iflow/agents/<agentId>/sessions`。
...
Skills 通过每个工作区的 `skills/` 文件夹按智能体隔离，共享 Skills 可从 `~/.iflow/skills` 获取。参阅[Skills：按智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置文件：`~/.iflow/iflow.json`（或 `IFLOW_CONFIG_PATH`）
- 状态目录：`~/.iflow`（或 `IFLOW_STATE_DIR`）
- 工作区：`~/.iflow/workspace`（或 `~/.iflow/workspace-<agentId>`）
- 智能体目录：`~/.iflow/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.iflow/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你不做任何配置，iFlow 会运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键格式为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.iflow/workspace`（设置了 `IFLOW_PROFILE` 时为 `~/.iflow/workspace-<profile>`）。
- 状态默认为 `~/.iflow/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认值：~/.iflow/credentials/whatsapp/personal
          // authDir: "~/.iflow/credentials/whatsapp/personal",
        },
        biz: {
          // 可选覆盖。默认值：~/.iflow/credentials/whatsapp/biz
