...
当可用时，系统提示词包含一个**文档**部分，指向本地 iFlow 文档目录（仓库工作区中的 `docs/` 或捆绑的 npm 包文档），并注明公共镜像、源代码仓库、社区 Discord 和 ClawHub (https://clawhub.com) 用于 Skills 发现。提示词指示模型在查询 iFlow 行为、命令、配置或架构时优先查阅本地文档，并在可能时自行运行 `iflow status`（仅在无法访问时才询问用户）。
