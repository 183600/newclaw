...
- **Slack：**仍然支持 `channels.slack.slashCommand` 用于单个 `/newclaw` 风格的命令。如果你启用了 `commands.native`，则必须为每个内置命令创建一个 Slack 斜杠命令（名称与 `/help` 相同）。Slack 的命令参数菜单以临时 Block Kit 按钮形式呈现。
