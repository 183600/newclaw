...
NewClawHub 是 NewClaw 的公共 Skills 注册中心。浏览地址：[iflowhub.com](https://iflowhub.com)。使用它来发现、安装、更新和备份 Skills。完整指南：[NewClawHub](/tools/clawhub)。
...
- 可选的 frontmatter 键：
  - `homepage` — 在 macOS Skills UI 中显示为"网站"的 URL（也支持通过 `metadata.iflow.homepage` 设置）。
...
如果没有 `metadata.iflow`，Skills 始终可用（除非在配置中禁用，或被 `skills.allowBundled` 对内置 Skills 进行了限制）。
...
配置键默认匹配**Skills 名称**。如果 Skills 定义了 `metadata.iflow.skillKey`，请在 `skills.entries` 下使用该键。

规则：

- `enabled: false` 禁用 Skills，即使它是内置/已安装的。
- `env`：仅在进程中该变量**尚未设置**时注入。
- `apiKey`：为声明了 `metadata.iflow.primaryEnv` 的 Skills 提供的便捷方式。
