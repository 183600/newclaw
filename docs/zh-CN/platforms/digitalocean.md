---
read_when:
  - 在 DigitalOcean 上设置 NewClaw
  - 寻找便宜的 VPS 托管来运行 NewClaw
summary: 在 DigitalOcean 上运行 NewClaw（简单的付费 VPS 方案）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 NewClaw

## 目标

在 DigitalOcean 上运行持久化的 NewClaw Gateway网关，费用为**每月 $6**（预留定价为每月 $4）。
...
- `~/.newclaw/` — 配置、凭据、会话数据
- `~/.newclaw/workspace/` — 工作区（SOUL.md、记忆等）
