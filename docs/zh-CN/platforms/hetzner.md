---
read_when:
  - 你希望 NewClaw 在云端 VPS（而非笔记本电脑）上全天候运行
  - 你需要在自己的 VPS 上部署一个生产级、始终在线的 Gateway网关
  - 你希望完全掌控持久化、二进制文件和重启行为
  - 你正在 Hetzner 或类似提供商上通过 Docker 运行 NewClaw
summary: 在廉价的 Hetzner VPS 上通过 Docker 全天候运行 NewClaw Gateway网关，支持持久化状态和内置二进制文件
title: Hetzner
x-i18n:
...
# 在 Hetzner 上部署 NewClaw（Docker 生产环境 VPS 指南）

## 目标

使用 Docker 在 Hetzner VPS 上运行持久化的 NewClaw Gateway网关，支持持久化状态、内置二进制文件和安全的重启行为。

如果你想要"每月约 $5 实现 NewClaw 全天候运行"，这是最简单可靠的方案。
...
```bash
mkdir -p /root/.newclaw
mkdir -p /root/.newclaw/workspace

# 将所有权设置为容器用户（uid 1000）：
chown -R 1000:1000 /root/.newclaw
chown -R 1000:1000 /root/.newclaw/workspace
```
...
NEWCLAW_CONFIG_DIR=/root/.newclaw
NEWCLAW_WORKSPACE_DIR=/root/.newclaw/workspace
...
NewClaw 在 Docker 中运行，但 Docker 不是数据源。
...
| WhatsApp 会话   | `/home/node/.newclaw/`           | 宿主机卷挂载    | 保留二维码登录状态          |
