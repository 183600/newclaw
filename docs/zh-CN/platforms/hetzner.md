...
```bash
mkdir -p /root/.iflow
mkdir -p /root/.iflow/workspace

# 将所有权设置为容器用户（uid 1000）：
chown -R 1000:1000 /root/.iflow
chown -R 1000:1000 /root/.iflow/workspace
```
...
| 组件            | 位置                           | 持久化机制      | 备注                        |
| --------------- | ------------------------------ | --------------- | --------------------------- |
| Gateway网关配置 | `/home/node/.iflow/`           | 宿主机卷挂载    | 包含 `newclaw.json`、令牌   |
...
| WhatsApp 会话   | `/home/node/.iflow/`           | 宿主机卷挂载    | 保留二维码登录状态          |
