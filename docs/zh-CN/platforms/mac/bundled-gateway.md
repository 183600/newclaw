...
iFlow.app 不再捆绑 Node/Bun 或 Gateway网关运行时。macOS 应用
...
- CLI 也可以安装它：`iflow gateway install`。

行为：

- "iFlow Active" 启用/禁用 LaunchAgent。
- 退出应用**不会**停止 Gateway网关（launchd 会保持其运行）。
- 如果配置端口上已有 Gateway网关在运行，应用会连接到该实例，而不是启动新的。

日志：

- launchd 标准输出/错误：`/tmp/iflow/iflow-gateway.log`
