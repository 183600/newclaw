...
- 设置稳定的调试 Bundle 标识符：`ai.iflow.mac.debug`
...
- 将构建元数据注入 Info.plist：`ClawBuildTimestamp`（UTC）和 `ClawGitCommit`（短哈希），以便"关于"面板可以显示构建信息、git 信息和调试/发布渠道。
...
- `ClawBuildTimestamp`：打包时的 ISO8601 UTC 时间
- `ClawGitCommit`：短 git 哈希（不可用时为 `unknown`）
