...
- 你仍然可以使用 [Gateway网关健康检查](/gateway/health) 中的 CLI 流程（`newclaw status`、`newclaw status --deep`、`newclaw health --json`），并跟踪 `/tmp/newclaw/iflow-*.log` 中的 `web-heartbeat` / `web-reconnect`。
