...
2. 确保 `iflow` 在非交互式 shell 的 PATH 中（如有需要，创建符号链接到 `/usr/local/bin` 或 `/opt/homebrew/bin`）。
...
2. 在 **NewClaw runs** 下，选择 **Remote over SSH** 并设置：
   ...
   - **CLI 路径**（高级）：可选的 `iflow` 可执行入口/二进制文件路径（广播时自动填充）。
...
- **exit 127 / not found**：`iflow` 不在非登录 shell 的 PATH 中。将其添加到 `/etc/paths`、你的 shell rc 文件中，或创建符号链接到 `/usr/local/bin`/`/opt/homebrew/bin`。
...
通过 `iflow` 和 `node.invoke` 在脚本中为每个通知选择声音，例如：
