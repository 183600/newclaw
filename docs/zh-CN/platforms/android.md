...
- 在 Gateway网关主机的 `~/.iflow/iflow.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway网关主机上设置 DNS-SD 区域（示例 `iflow.internal.`）并发布 `_iflow-gw._tcp` 记录。
...
1. 在 Gateway网关主机上创建 `~/.iflow/workspace/canvas/index.html`。
