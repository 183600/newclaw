...
summary: 在 exe.dev 上运行 NewClaw Gateway网关（虚拟机 + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：在 exe.dev 虚拟机上运行 NewClaw Gateway网关，通过以下地址从你的笔记本访问：`https://<vm-name>.exe.xyz`
...
Shelley 是 [exe.dev](https://exe.dev) 的智能体，可以通过我们的提示词即时安装 NewClaw。使用的提示词如下：
...
提示：保持此虚拟机为**有状态**。NewClaw 将状态存储在 `~/.newclaw/` 和 `~/.newclaw/workspace/` 下。
...
## 4）配置 nginx 将 NewClaw 代理到端口 8000
...
访问 `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`。使用 `newclaw devices list` 和 `newclaw device approve` 审批设备。如果不确定，可以从浏览器使用 Shelley！
