...
summary: 在 exe.dev 上运行 iFlow Gateway网关（虚拟机 + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：在 exe.dev 虚拟机上运行 iFlow Gateway网关，通过以下地址从你的笔记本访问：`https://<vm-name>.exe.xyz`
...
Shelley 是 [exe.dev](https://exe.dev) 的智能体，可以通过我们的提示词即时安装 iFlow。使用的提示词如下：
...
提示：保持此虚拟机为**有状态**。iFlow 将状态存储在 `~/.iflow/` 和 `~/.iflow/workspace/` 下。
...
访问 `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`。使用 `iflow devices list` 和 `iflow device approve` 审批设备。如果不确定，可以从浏览器使用 Shelley！
