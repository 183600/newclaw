...
### 第二层：Gateway网关 + 开发智能体冒烟测试（"@iflow"实际做什么）
...
- `NEWCLAW_CONFIG_DIR=...`（默认：`~/.newclaw`）挂载到 `/home/node/.iflow`
- `NEWCLAW_WORKSPACE_DIR=...`（默认：`~/.newclaw/workspace`）挂载到 `/home/node/.iflow/workspace`
