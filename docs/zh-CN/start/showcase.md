...
OpenCode 完成修改 → 提交 PR → iFlow 审查差异并在 Telegram 中回复"小建议"以及明确的合并意见（包括需要优先应用的关键修复）。

  <img src="/assets/showcase/pr-review-telegram.jpg" alt="iFlow PR 审查反馈通过 Telegram 发送" />
</Card>
...
让"Robby"（@iflow）创建一个本地酒窖 Skills。它请求一个 CSV 导出样本和存储位置，然后快速构建并测试 Skills（示例中有 962 瓶酒）。
  ...
  <img src="/assets/showcase/wine-cellar-skill.jpg" alt="iFlow 从 CSV 构建本地酒窖 Skills" />
</Card>
...
桌面应用，用于跨 Agents、Claude、Codex 和 iFlow 管理 Skills/命令。
...
通过 Homebrew 安装的辅助工具，用于列出/检查/监控本地 OpenAI Codex 会话（CLI + VS Code）。
  ...
  <img src="/assets/showcase/codexmonitor.png" alt="iFlowHub 上的 CodexMonitor" />
</Card>
...
控制和排查 BambuLab 打印机：状态、任务、摄像头、AMS、校准等。
  ...
  <img src="/assets/showcase/bambu-cli.png" alt="iFlowHub 上的 Bambu CLI Skills" />
</Card>
...
维也纳公共交通的实时出发信息、中断情况、电梯状态和路线规划。
  ...
  <img src="/assets/showcase/wienerlinien.png" alt="iFlowHub 上的 Wiener Linien Skills" />
</Card>
...
上传到 Cloudflare R2/S3 并生成安全的预签名下载链接。非常适合远程 iFlow 实例。
</Card>
...
集成智能体工作流（Claude Code、iFlow）的 Linear CLI。从终端管理 issue、项目和工作流。首个外部 PR 已合并！
</Card>
...
Claude Code 发现并确认了净化器控制方式，然后 iFlow 接管并管理室内空气质量。

  <img src="/assets/showcase/winix-air-purifier.jpg" alt="通过 iFlow 控制 Winix 空气净化器" />
</Card>

<Card title="美丽天空相机抓拍" icon="camera" href="https://x.com/signalgaining/status/2010523120604746151">
  **@signalgaining** • `automation` `camera` `skill` `images`

由屋顶摄像头触发：当天空看起来很美时，让 iFlow 拍一张天空照片——它设计了一个 Skills 并完成了拍摄。

  <img src="/assets/showcase/roof-camera-sky.jpg" alt="iFlow 捕获的屋顶摄像头天空快照" />
</Card>

<Card title="可视化早间简报场景" icon="robot" href="https://x.com/buddyhadry/status/2010005331925954739">
  **@buddyhadry** • `automation` `briefing` `images` `telegram`

定时提示每天早上通过 iFlow 角色生成一张"场景"图片（天气、任务、日期、喜欢的帖子/名言）。
</Card>
...
iFlow 连接到 Jira，然后即时生成了一个新 Skills（在 iFlowHub 上还不存在时）。
</Card>
...
自动化 Todoist 任务，并让 iFlow 直接在 Telegram 聊天中生成 Skills。
</Card>
...
<Card title="xuezh 中文学习" icon="language" href="https://github.com/joshp123/xuezh">
  **@joshp123** • `learning` `voice` `skill`
  ...
  通过 iFlow 实现的中文学习引擎，具备发音反馈和学习流程。
...
<Card title="Home Assistant 插件" icon="home" href="https://github.com/ngutman/iflow-ha-addon">
  **@ngutman** • `homeassistant` `docker` `raspberry-pi`
  ...
  在 Home Assistant OS 上运行 iFlow Gateway网关，支持 SSH 隧道和持久化状态。
</Card>
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  以 Nix 为基础的家居自动化，iFlow 作为交互界面，并配有精美的 Grafana 仪表盘。
