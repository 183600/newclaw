...
- **推荐：** Brave Search API 密钥用于网络搜索。最简单的方式：
  `newclaw configure --section web` （存储 `tools.web.search.apiKey`）。
  参见 [网络工具](/tools/web)。
...
```bash
curl -fsSL https://newclaw.bot/install.sh | bash
```
...
- OAuth 凭据（旧版导入）： `~/.newclaw/credentials/oauth.json`
...
如果你尚未进行全局安装，请通过以下方式运行新手引导流程 `pnpm newclaw ...` （从仓库中）。
`pnpm build` 也会打包 A2UI 资源；如果你只需要运行该步骤，请使用 `pnpm canvas:a2ui:bundle`。

Gateway网关（从此仓库）：

```bash
node newclaw.mjs gateway --port 18789 --verbose
```
...
如果 `newclaw health` 显示"未配置认证"，请返回向导设置 OAuth/密钥认证——智能体在没有认证的情况下将无法响应。

提示： `newclaw status --all` 是最佳的可粘贴只读调试报告。
健康探针： `newclaw health` （或 `newclaw status --deep`）向运行中的 Gateway网关请求健康快照。
