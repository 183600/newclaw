...
如果安装器完成但在新终端中找不到 `iflow`，通常是 Node/npm PATH 问题。参见：[安装](/install#nodejs--npm-path-sanity)。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g iflow@latest`
...
此脚本将 `iflow` 安装到前缀目录（默认：`~/.newclaw`），同时在该前缀下安装专用的 Node 运行时，因此可以在不想改动系统 Node/npm 的机器上使用。
