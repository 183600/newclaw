...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [安装和设置 NewClaw 的推荐方式是什么？](#whats-the-recommended-way-to-install-and-set-up-iflow)
  ...
  - [无法访问 docs.newclaw.ai（SSL 错误），怎么办？](#i-cant-access-docsclawai-ssl-error-what-now)
...
- [如何安装 beta 版本，beta 和 dev 有什么区别？](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  ...
  - [Windows 安装提示找不到 git 或无法识别 iflow](#windows-install-says-git-not-found-or-iflow-not-recognized)
  - [文档没有解答我的问题——如何获得更好的答案？](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [如何在 Linux 上安装 NewClaw？](#how-do-i-install-iflow-on-linux)
  - [如何在 VPS 上安装 NewClaw？](#how-do-i-install-iflow-on-a-vps)
  - [云/VPS 安装指南在哪里？](#where-are-the-cloudvps-install-guides)
  - [可以让 NewClaw 自行更新吗？](#can-i-ask-iflow-to-update-itself)
  ...
  - [如果我买了 Mac mini 运行 NewClaw，能连接到我的 MacBook Pro 吗？](#if-i-buy-a-mac-mini-to-run-iflow-can-i-connect-it-to-my-macbook-pro)
  - [可以使用 Bun 吗？](#can-i-use-bun)
  - [Telegram：`allowFrom` 填什么？](#telegram-what-goes-in-allowfrom)
  - [多人能否使用同一个 WhatsApp 号码配合不同的 NewClaw 实例？](#can-multiple-people-use-one-whatsapp-number-with-different-iflow-instances)
  ...
  - [在专用机器上运行 NewClaw 有多重要？](#how-important-is-it-to-run-iflow-on-a-dedicated-machine)
  - [VPS 的最低要求和推荐操作系统是什么？](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [可以在虚拟机中运行 NewClaw 吗？有什么要求？](#can-i-run-iflow-in-a-vm-and-what-are-the-requirements)
- [什么是 NewClaw？](#what-is-iflow)
  - [用一段话描述 NewClaw？](#what-is-iflow-in-one-paragraph)
  - [价值主张是什么？](#whats-the-value-proposition)
  - [刚设置好，应该先做什么？](#i-just-set-it-up-what-should-i-do-first)
  - [NewClaw 日常最常用的五个场景是什么？](#what-are-the-top-five-everyday-use-cases-for-iflow)
  - [NewClaw 能否帮助 SaaS 进行获客、外联、广告和博客？](#can-iflow-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills 与自动化](#skills-and-automation)
  ...
  - [NewClaw 能否按计划或在后台持续运行任务？](#can-iflow-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [磁盘上的文件位置](#where-things-live-on-disk)
  - [NewClaw 使用的所有数据都保存在本地吗？](#is-all-data-used-with-iflow-saved-locally)
  - [NewClaw 将数据存储在哪里？](#where-does-iflow-store-its-data)
  ...
  - [如何完全卸载 NewClaw？](#how-do-i-completely-uninstall-iflow)
...
- [配置基础](#config-basics)
  ...
  - [NewClaw 浏览器可以无头运行吗？](#can-the-iflow-browser-run-headless)
...
- [远程 Gateway网关与节点](#remote-gateways-nodes)
  ...
  - [两个 NewClaw 实例（本地 + VPS）可以互相通信吗？](#can-two-iflow-instances-talk-to-each-other-local-vps)
...
- [环境变量和 .env 加载](#env-vars-and-env-loading)
  - [NewClaw 如何加载环境变量？](#how-does-iflow-load-environment-variables)
...
- [会话与多聊天](#sessions-multiple-chats)
  ...
  - [能否创建一个 NewClaw 实例团队——一个 CEO 和多个智能体？](#is-there-a-way-to-make-a-team-of-iflow-instances-one-ceo-and-many-agents)
  - [为什么上下文在任务中途被截断了？如何防止？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 NewClaw 但保留安装？](#how-do-i-completely-reset-iflow-but-keep-it-installed)
  ...
  - [为什么 NewClaw 不在群组中回复？](#why-doesnt-iflow-reply-in-a-group)
...
- [模型：默认值、选择、别名、切换](#models-defaults-selection-aliases-switching)
  ...
  - [NewClaw、Flawd 和 Krill 使用什么模型？](#what-do-iflow-flawd-and-krill-use-for-models)
...
- [Gateway网关：端口、"已在运行"和远程模式](#gateway-ports-already-running-and-remote-mode)
  ...
  - [如何以远程模式运行 NewClaw（客户端连接到其他位置的 Gateway网关）？](#how-do-i-run-iflow-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [日志与调试](#logging-and-debugging)
  ...
  - [我在 Windows 上关闭了终端——如何重启 NewClaw？](#i-closed-my-terminal-on-windows-how-do-i-restart-iflow)
...
- [安全与访问控制](#security-and-access-control)
  - [将 NewClaw 暴露给入站私信安全吗？](#is-it-safe-to-expose-iflow-to-inbound-dms)
...
相关：[迁移](/install/migrating)、[磁盘上的文件位置](/help/faq#where-does-iflow-store-its-data)、
...
一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.newclaw.ai`。禁用该功能或将 `docs.newclaw.ai` 加入白名单，然后重试。更多详情：[故障排除](/help/troubleshooting#docsclawai-shows-an-ssl-error-comcastxfinity)。
...
### Windows 安装提示找不到 git 或无法识别 iflow
...
使用 **NewClawHub**（CLI）或将 Skills 放入你的工作区。macOS Skills UI 在 Linux 上不可用。
浏览 Skills：https://iflowhub.com。

安装 NewClawHub CLI（选择一个包管理器）：
...
不能直接运行。macOS Skills 受 `metadata.iflow.os` 和所需二进制文件限制，Skills 只有在 **Gateway网关主机**上符合条件时才会出现在系统提示中。在 Linux 上，`darwin` 专用 Skills（如 `imsg`、`apple-notes`、`apple-reminders`）不会加载，除非你覆盖限制。

你有三种支持的模式：

**方案 A - 在 Mac 上运行 Gateway网关（最简单）。**
在 macOS 二进制文件所在的地方运行 Gateway网关，然后从 Linux 通过[远程模式](#how-do-i-run-iflow-in-remote-mode-client-connects-to-a-gateway-elsewhere)或 Tailscale 连接。Skills 正常加载，因为 Gateway网关主机是 macOS。
...
3. 覆盖 Skills 元数据（工作区或 `~/.newclaw/skills`）以允许 Linux：
   ...
   metadata: { "iflow": { "os": ["darwin", "linux"], "requires": { "bins": ["imsg"] } } }
   ---
   ```
...
NewClawHub 安装到当前目录下的 `./skills`（或回退到你配置的 NewClaw 工作区）；NewClaw 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.newclaw/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [NewClawHub](/tools/clawhub)。
...
- 控制 UI 将令牌存储在浏览器 localStorage 键 `iflow.control.settings.v1` 中。
...
配置文件还会为服务名称添加后缀（`bot.molt.<profile>`；旧版 `com.iflow.*`、`newclaw-gateway-<profile>.service`、`NewClaw Gateway网关 (<profile>)`）。
...
智能体的出站附件必须包含 `MEDIA:<path-or-url>` 行（独占一行）。参阅 [NewClaw 助手设置](/start/iflow)和 [Agent send](/tools/agent-send)。
