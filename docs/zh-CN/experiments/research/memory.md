---
read_when:
  - 设计超越每日 Markdown 日志的工作区记忆（~/.iflow/workspace）
  - 决策：独立 CLI 还是深度集成 iFlow
...
目标：Clawd 风格的工作区（`agents.defaults.workspace`，默认 `~/.iflow/workspace`），其中"记忆"以每日一个 Markdown 文件（`memory/YYYY-MM-DD.md`）加上一小组稳定文件（如 `memory.md`、`SOUL.md`）的形式存储。
...
保持 `~/.iflow/workspace` 作为规范的人类可读记忆。
