...
运行与 iFlow Gateway网关通信的 ACP（Agent Client Protocol）桥接。
...
当 IDE（或其他客户端）使用 Agent Client Protocol 并且你希望它驱动 iFlow Gateway网关会话时，请使用 ACP。

1. 确保 Gateway网关正在运行（本地或远程）。
2. 配置 Gateway网关目标（通过配置文件或标志）。
3. 将你的 IDE 配置为通过 stdio 运行 `iflow acp`。
...
在 Zed 中，打开 Agent 面板并选择 "iFlow ACP" 来开始一个对话线程。
