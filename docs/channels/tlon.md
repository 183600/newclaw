...
Tlon is a decentralized messenger built on Urbit. NewClaw connects to your Urbit ship and can
...
```bash
newclaw plugins install @newclaw/tlon
```

Local checkout (when running from a git repo):

```bash
newclaw plugins install ./extensions/tlon
```
...
Use these with `newclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, NewClaw replies in-thread.
