...
Tlon is a decentralized messenger built on Urbit. iFlow connects to your Urbit ship and can
...
```bash
iflow plugins install @iflow/tlon
```

Local checkout (when running from a git repo):

```bash
iflow plugins install ./extensions/tlon
```
...
Use these with `iflow message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, iFlow replies in-thread.
