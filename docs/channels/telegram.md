...
iFlow can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, iFlow retries the same message as plain text.

## Commands (native + custom)

iFlow registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; iFlow does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `iflow logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). iFlow can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. iFlow:
...
Private chats can include `message_thread_id` in some edge cases. iFlow keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2. Run `iflow logs --follow` and look for `from.id`.
...
iFlow defaults to audio files for backward compatibility.
...
iFlow supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, iFlow handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, iFlow caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.iflow/telegram/sticker-cache.json`
...
iFlow uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, iFlow:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by iFlow)
...
- Example: `iflow message send --channel telegram --target 123456789 --message "hi"`.
...
- `iflow channels status` shows a warning when config expects unmentioned group messages.
- `iflow channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `iflow logs --follow` (look for "skipping group message")
...
- Upgrade to a iFlow build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
