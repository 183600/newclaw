...
- Talk to iFlow via Discord DMs or guild channels.
...
3. Configure iFlow with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `iflow pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running iFlow in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents iFlow needs
...
Discord uses numeric ids everywhere; iFlow config prefers ids.
...
- First: run `iflow doctor` and `iflow channels status --probe` (actionable warnings + quick audits).
...
When enabled, iFlow uses the member identity for allowlists and labels the
...
- On startup, iFlow resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror iFlow’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; iFlow enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`iflow gateway --force`) after confirming no other processes own the Discord session.
