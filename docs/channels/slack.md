...
3. Set tokens for NewClaw and start the gateway.
...
7. Slash Commands → create `/newclaw` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
NewClaw can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, NewClaw falls back to the user token.
...
```json
{
  "display_information": {
    "name": "NewClaw",
    "description": "Slack connector for NewClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "NewClaw",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/newclaw",
        "description": "Send a message to NewClaw",
        "should_escape": false
      }
    ]
  },
...
By default, NewClaw replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, NewClaw infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `newclaw pairing approve slack <code>`.
...
- On startup, NewClaw resolves channel/user names in allowlists to IDs (when tokens allow)
