...
3. Set tokens for iFlow and start the gateway.
...
iFlow can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, iFlow falls back to the user token.
...
```json
{
  "display_information": {
    "name": "iFlow",
    "description": "Slack connector for iFlow"
  },
  "features": {
    "bot_user": {
      "display_name": "iFlow",
...
By default, iFlow replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, iFlow infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `iflow pairing approve slack <code>`.
...
- On startup, iFlow resolves channel/user names in allowlists to IDs (when tokens allow)
