...
```bash
newclaw plugins install @newclaw/msteams
```

Local checkout (when running from a git repo):

```bash
newclaw plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
NewClaw will offer the local install path automatically.
...
3. Configure NewClaw with those credentials.
...
- Talk to NewClaw via Teams DMs, group chats, or channels.
...
- On startup, NewClaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.newclaw/newclaw.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring NewClaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                   |
   | ------------------ | ------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `newclaw-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `newclaw plugins install @newclaw/msteams`
   - From a local checkout: `newclaw plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "NewClaw in Teams", "full": "NewClaw in Teams" },
...
NewClaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, NewClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/NewClawShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

NewClaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `newclaw message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.newclaw/msteams-polls.json`.
...
```bash
newclaw message send --channel msteams \
...
newclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
newclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
newclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
newclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
