...
```bash
iflow plugins install @iflow/msteams
```

Local checkout (when running from a git repo):

```bash
iflow plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
iFlow will offer the local install path automatically.
...
3. Configure iFlow with those credentials.
...
- Talk to iFlow via Teams DMs, group chats, or channels.
...
- On startup, iFlow resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.iflow/iflow.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring iFlow, you need to create an Azure Bot resource.
...
1. **Install the Microsoft Teams plugin**
   - From npm: `iflow plugins install @iflow/msteams`
   - From a local checkout: `iflow plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "iFlow in Teams", "full": "iFlow in Teams" },
...
iFlow handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, iFlow only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
iFlow sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `iflow message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.iflow/msteams-polls.json`.
...
```bash
iflow message send --channel msteams \
...
iflow message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
iflow message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
iflow message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
iflow message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
