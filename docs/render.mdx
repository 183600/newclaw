...
Deploy NewClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/newclaw/newclaw"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run NewClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: NEWCLAW_STATE_DIR
        value: /data/.newclaw
      - key: NEWCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: NEWCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/newclaw`.
...
If you use the original NewClaw repository, Render will not auto-deploy your NewClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For NewClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any NewClaw host.
