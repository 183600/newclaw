...
Deploy iFlow on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/iflow/iflow"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run iFlow:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: IFLOW_STATE_DIR
        value: /data/.iflow
      - key: IFLOW_WORKSPACE_DIR
        value: /data/workspace
      - key: IFLOW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/iflow`.
...
If you use the original iFlow repository, Render will not auto-deploy your iFlow. To update it, run a manual Blueprint sync from the dashboard.
...
For iFlow, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any iFlow host.
