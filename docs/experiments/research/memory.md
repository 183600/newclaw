...
read_when:
  - Designing workspace memory (~/.newclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep NewClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.newclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.newclaw/workspace` as canonical human-readable memory.
...
```
~/.newclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in NewClaw**, but keep a separable core library.

### Why integrate into NewClaw?

- NewClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `newclaw memory recall "…" --k 25 --since 30d`
  - `newclaw memory reflect --since 7d`
...
Pragmatic take for `~/.newclaw/workspace`:
