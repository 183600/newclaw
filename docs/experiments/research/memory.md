...
read_when:
  - Designing workspace memory (~/.iflow/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep iFlow integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.iflow/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.iflow/workspace` as canonical human-readable memory.
...
```
~/.iflow/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in iFlow**, but keep a separable core library.

### Why integrate into iFlow?
...
- You want the agent itself to call the tools:
  - `iflow memory recall "…" --k 25 --since 30d`
  - `iflow memory reflect --since 7d`
...
Pragmatic take for `~/.iflow/workspace`:
