...
NewClaw features that can generate provider usage or paid API calls.
...
- `newclaw status --usage` and `newclaw channels list` show provider **usage windows**
...
NewClaw can pick up credentials from:
...
- `newclaw status --usage`
- `newclaw models status --json`
...
`newclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
