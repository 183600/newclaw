...
iFlow features that can generate provider usage or paid API calls.
...
- `iflow status --usage` and `iflow channels list` show provider **usage windows**
...
iFlow can pick up credentials from:
...
`iflow models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
