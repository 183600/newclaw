...
This document explains how NewClaw manages sessions end-to-end:
...
NewClaw is designed around a single **Gateway process** that owns session state.
...
NewClaw persists sessions in two layers:
...
- Transcripts: `~/.newclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

NewClaw resolves these via `src/config/sessions.ts`.
...
NewClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (NewClaw consumes the events, but Pi decides when to compact).
...
NewClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, NewClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, NewClaw leaves it alone.
...
- `newclaw status` (CLI)
- `newclaw sessions` / `sessions --json`
...
NewClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- NewClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, NewClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
NewClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but NewClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `newclaw status`.
