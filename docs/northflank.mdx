...
Deploy NewClaw on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy NewClaw](https://northflank.com/stacks/deploy-newclaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy NewClaw now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the NewClaw template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the NewClaw service.
8. Open the public NewClaw URL and complete setup at `/setup`.
9. Open the Control UI at `/newclaw`.

## What you get

- Hosted NewClaw Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/newclaw`
