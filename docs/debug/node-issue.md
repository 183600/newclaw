...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in iFlow
...
Running iFlow via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `iflow status` and `gateway:watch` worked.
