...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in NewClaw
...
Running NewClaw via Node with `tsx` fails at startup with:

```
[newclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `newclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch newclaw.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node newclaw.mjs status` works on Node 25.
