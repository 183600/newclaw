...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/claw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Claw Re-Auth" -c "After getting key, run: ssh $SERVER '~/claw/scripts/mobile-reauth.sh'" --id claw-auth
        ;;
    *)
