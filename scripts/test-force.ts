...
function runTests() {
  const isolatedLock =
    process.env.IFLOW_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `iflow-gateway.lock.test.${Date.now()}`);
...
function main() {
  const port = Number.parseInt(process.env.IFLOW_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
