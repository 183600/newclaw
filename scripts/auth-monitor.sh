...
#
# Suggested cron: */30 * * * * /home/admin/newclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send NewClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., newclaw-alerts)
...
STATE_FILE="$HOME/.newclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via NewClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use newclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via NewClaw to $NOTIFY_PHONE..."
            newclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! NewClaw is down. Run: ssh l36 '~/newclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
