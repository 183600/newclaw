...
#
# Suggested cron: */30 * * * * /home/admin/iflow/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send iFlow notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., iflow-alerts)
...
STATE_FILE="$HOME/.iflow/auth-monitor-state"
...
send_notification() {
    ...
    # Send via iFlow if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use iflow
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via iFlow to $NOTIFY_PHONE..."
            iflow send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! iFlow is down. Run: ssh l36 '~/iflow/scripts/mobile-reauth.sh'" "urgent"
    exit 1
