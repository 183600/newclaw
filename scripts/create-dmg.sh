...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "NewClaw")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "NewClaw")
...
DMG_TEMP="$(mktemp -d /tmp/newclaw-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/NewClaw/Resources/NewClaw.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/newclaw-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/newclaw-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/newclaw-dmg-limits.txt
