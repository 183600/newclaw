...
const shardOverride = Number.parseInt(process.env.NEWCLAW_TEST_SHARDS ?? "", 10);
...
const windowsCiArgs = isWindowsCi
  ...
  : [];
const overrideWorkers = Number.parseInt(process.env.NEWCLAW_TEST_WORKERS ?? "", 10);
...
// Reduced worker count for faster execution
...
const runOnce = (entry, extraArgs = []) =>
  new Promise((resolve) => {
    // Added test timeout and no coverage for faster execution
    const args = maxWorkers
      ...
      : [...entry.args, "--no-coverage", "--testTimeout=30000", ...windowsCiArgs, ...extraArgs];
