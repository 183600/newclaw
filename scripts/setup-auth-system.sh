#!/bin/bash
# Setup NewClaw Auth Management System
...
echo "=== NewClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'newclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  NewClaw message: Send warning via NewClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/newclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/newclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/newclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now newclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status newclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
