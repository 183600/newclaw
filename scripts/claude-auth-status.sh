...
# Checks both Claude Code and NewClaw auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
NEWCLAW_AUTH="$HOME/.newclaw/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    newclaw models status --json 2>/dev/null || true
}
...
check_newclaw_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$NEWCLAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    newclaw_status=$(check_newclaw_auth 2>/dev/null || true)

    claude_expires=0
    newclaw_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        newclaw_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        newclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$NEWCLAW_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$newclaw_status" \
        --arg be "$newclaw_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            newclaw: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$newclaw_status" == EXPIRED* ]] || [[ "$newclaw_status" == MISSING* ]]; then
        echo "NEWCLAW_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$newclaw_status" == EXPIRING* ]]; then
        echo "NEWCLAW_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "NewClaw Auth (~/.newclaw/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$NEWCLAW_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$NEWCLAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'newclaw doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
if systemctl --user is-active newclaw >/dev/null 2>&1; then
    echo -e "NewClaw service: ${GREEN}running${NC}"
else
    echo -e "NewClaw service: ${RED}NOT running${NC}"
fi
