...
# Checks both Claude Code and Claw auth status
...
fetch_models_status_json() {
    claw models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    claw_status=$(check_claw_auth 2>/dev/null || true)

    claude_expires=0
    claw_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        claw_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        claw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$IFLOW_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            claw: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$claw_status" == EXPIRED* ]] || [[ "$claw_status" == MISSING* ]]; then
        echo "IFLOW_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$claw_status" == EXPIRING* ]]; then
        echo "IFLOW_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Claw Auth (~/.iflow/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'claw doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
if systemctl --user is-active claw >/dev/null 2>&1; then
    echo -e "Claw service: ${GREEN}running${NC}"
else
    echo -e "Claw service: ${RED}NOT running${NC}"
fi
