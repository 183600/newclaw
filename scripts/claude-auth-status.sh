...
# Checks both Claude Code and iFlow auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
IFLOW_AUTH="$HOME/.iflow/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    iflow models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    iflow_status=$(check_iflow_auth 2>/dev/null || true)

    claude_expires=0
    iflow_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        iflow_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        iflow_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$IFLOW_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$iflow_status" \
        --arg be "$iflow_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            iflow: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$iflow_status" == EXPIRED* ]] || [[ "$iflow_status" == MISSING* ]]; then
        echo "IFLOW_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$iflow_status" == EXPIRING* ]]; then
        echo "IFLOW_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "iFlow Auth (~/.iflow/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$IFLOW_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$IFLOW_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'iflow doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
if systemctl --user is-active iflow >/dev/null 2>&1; then
    echo -e "iFlow service: ${GREEN}running${NC}"
else
    echo -e "iFlow service: ${RED}NOT running${NC}"
fi
