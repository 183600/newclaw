...
IMAGE_NAME="newclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/newclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/newclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/newclaw-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/newclaw-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.newclaw/workspace"
    config_path="$HOME/.newclaw/newclaw.json"
    sessions_dir="$HOME/.newclaw/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.newclaw/newclaw.json" <<'"'"'JSON'"'"'
{
