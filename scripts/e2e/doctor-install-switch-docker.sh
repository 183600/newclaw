...
IMAGE_NAME="newclaw-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/newclaw-bin:$PATH"
  mkdir -p /tmp/newclaw-bin

  cat > /tmp/newclaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/newclaw-bin/systemctl

  cat > /tmp/newclaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/newclaw-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/newclaw"
  npm_entry="/tmp/npm-prefix/lib/node_modules/newclaw/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/newclaw.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/newclaw-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/newclaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
