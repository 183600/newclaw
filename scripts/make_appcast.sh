...
ZIP=${1:?"Usage: $0 NewClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/newclaw/newclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^NewClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/newclaw/newclaw/releases/download/v${VERSION}/"}
