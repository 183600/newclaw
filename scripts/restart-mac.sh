...
GATEWAY_WAIT_SECONDS="${IFLOW_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.iflow/disable-launchagent"
ATTACH_ONLY=1
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.iflow.mac 2>/dev/null || true
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".iflow", "iflow.json"), "utf8");
