...
BASE_IMAGE="${BASE_IMAGE:-claw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-claw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=claw.sandbox=1)
NOTE
