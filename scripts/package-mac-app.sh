...
# Build and bundle Claw into a minimal .app we can open.
# Outputs to dist/Claw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Claw.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/claw/claw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Claw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Claw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Claw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Claw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Claw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Claw/Resources/Claw.icns" "$APP_ROOT/Contents/Resources/Claw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Claw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ClawKit resources"
CLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ClawKit_ClawKit.bundle"
if [ -d "$CLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ClawKit_ClawKit.bundle"
  cp -R "$CLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ClawKit_ClawKit.bundle"
else
  echo "WARN: ClawKit resource bundle not found at $CLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Claw"
killall -q Claw 2>/dev/null || true
