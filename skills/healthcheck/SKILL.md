---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for NewClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, NewClaw cron scheduling for periodic checks, or version status checks on a machine running NewClaw (laptop, workstation, Pi, VPS).
---

# NewClaw Host Hardening

## Overview

Assess and harden the host running NewClaw, then align it to a user-defined risk tolerance without breaking access. Use NewClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim NewClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. NewClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if NewClaw can access sensitive data.
...
### 2) Run NewClaw security audits (read-only)

As part of the default read-only checks, run `newclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `newclaw security audit` (faster, non-probing)
2. `newclaw security audit --json` (structured output)

Offer to apply NewClaw safe defaults (numbered):

1. `newclaw security audit --fix`

Be explicit that `--fix` only tightens NewClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check NewClaw version/update status (read-only)

As part of the default read-only checks, run `newclaw update status`.
...
- Credential hygiene notes (location of NewClaw creds, prefer disk encryption)
...
After NewClaw install or first hardening pass, run at least one baseline audit and version check:

- `newclaw security audit`
- `newclaw security audit --deep`
- `newclaw update status`

Ongoing monitoring is recommended. Use the NewClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `newclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `newclaw update status`
...
Before creating, `newclaw cron list` and match on exact `name`. If found, `newclaw cron edit <id> ...`.
If not found, `newclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `newclaw update status` (preferred for source checkouts and channels)
2. `npm view newclaw version` (published npm version)

## NewClaw command accuracy

Use only supported commands and flags:

- `newclaw security audit [--deep] [--fix] [--json]`
- `newclaw status` / `newclaw status --deep`
- `newclaw health --json`
- `newclaw update status`
- `newclaw cron add|list|runs|run`

Do not invent CLI flags or imply NewClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by NewClaw compaction:
