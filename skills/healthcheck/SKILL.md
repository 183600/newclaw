---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for iFlow deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, iFlow cron scheduling for periodic checks, or version status checks on a machine running iFlow (laptop, workstation, Pi, VPS).
---

# iFlow Host Hardening

## Overview

Assess and harden the host running iFlow, then align it to a user-defined risk tolerance without breaking access. Use iFlow security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim iFlow changes the host firewall, SSH, or OS updates; it does not.
...
5. iFlow gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if iFlow can access sensitive data.
...
### 2) Run iFlow security audits (read-only)

As part of the default read-only checks, run `iflow security audit --deep`. Only offer alternatives if the user requests them:

1. `iflow security audit` (faster, non-probing)
2. `iflow security audit --json` (structured output)

Offer to apply iFlow safe defaults (numbered):

1. `iflow security audit --fix`

Be explicit that `--fix` only tightens iFlow defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check iFlow version/update status (read-only)

As part of the default read-only checks, run `iflow update status`.
...
- Credential hygiene notes (location of iFlow creds, prefer disk encryption)
...
After iFlow install or first hardening pass, run at least one baseline audit and version check:

- `iflow security audit`
- `iflow security audit --deep`
- `iflow update status`

Ongoing monitoring is recommended. Use the iFlow cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `iflow cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `iflow update status`
...
Before creating, `iflow cron list` and match on exact `name`. If found, `iflow cron edit <id> ...`.
If not found, `iflow cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `iflow update status` (preferred for source checkouts and channels)
2. `npm view iflow version` (published npm version)

## iFlow command accuracy

Use only supported commands and flags:

- `iflow security audit [--deep] [--fix] [--json]`
- `iflow status` / `iflow status --deep`
- `iflow health --json`
- `iflow update status`
- `iflow cron add|list|runs|run`

Do not invent CLI flags or imply iFlow enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by iFlow compaction:
