// Test the fixed special character processing
function debugSpecialCharProcessingFixed(text) {
  ...
  while (i < text.length) {
    ...
    // Check for special character closing tags with fixed condition
    if (
      (i + 8 < text.length && text.substring(i, i + 8) === "thinking\u0111") ||
      (i + 7 < text.length && text.substring(i, i + 7) === "thought\u0111") ||
      (i + 11 < text.length && text.substring(i, i + 11) === "antthinking\u0111")
    ) {
      let endPos;
      let tagType;
      if (i + 8 < text.length && text.substring(i, i + 8) === "thinking\u0111") {
        endPos = i + 8;
        tagType = "thinkingđ";
      } else if (i + 7 < text.length && text.substring(i, i + 7) === "thought\u0111") {
        endPos = i + 7;
        tagType = "thoughtđ";
      } else if (i + 11 < text.length && text.substring(i, i + 11) === "antthinking\u0111") {
        endPos = i + 11;
...
console.log("=== Special Character Processing Debug (Fixed) ===");
const overlappingText = "Before Đthinking nested <thinking>content</thinking> thinkingđ after.";
const result = debugSpecialCharProcessingFixed(overlappingText);
