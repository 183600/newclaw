...
private let chatUILogger = Logger(subsystem: "ai.iflow", category: "iFlowChatUI")

@MainActor
@Observable
public final class iFlowChatViewModel {
    public private(set) var messages: [iFlowChatMessage] = []
    ...
    public var attachments: [iFlowPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [iFlowChatPendingToolCall] = []
    public private(set) var sessions: [iFlowChatSessionEntry] = []
    private let transport: any iFlowChatTransport
    ...
    private var pendingToolCallsById: [String: iFlowChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any iFlowChatTransport) {
    ...
    public var sessionChoices: [iFlowChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [iFlowChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [iFlowChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: iFlowPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [iFlowChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: iFlowChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [iFlowChatMessage]) -> [iFlowChatMessage] {
        var result: [iFlowChatMessage] = []
    ...
    private static func dedupeKey(for message: iFlowChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [iFlowChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> iFlowChatAttachmentPayload in
            iFlowChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> iFlowChatSessionEntry {
        iFlowChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: iFlowChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: iFlowChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: iFlowAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = iFlowChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> iFlowPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
