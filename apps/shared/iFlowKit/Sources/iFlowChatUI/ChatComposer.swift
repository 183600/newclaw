...
@MainActor
struct iFlowChatComposer: View {
    @Bindable var viewModel: iFlowChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(iFlowChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(iFlowChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \iFlowPendingAttachment.id)
                { (att: iFlowPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            iFlowPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(iFlowChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(iFlowChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(iFlowChatTheme.subtleCard)
        .clipShape(Capsule())
    }
