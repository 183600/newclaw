...
private final class TestChatTransport: @unchecked Sendable, iFlowChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [iFlowChatHistoryPayload]
    private let sessionsResponses: [iFlowChatSessionsListResponse]

    private let stream: AsyncStream<iFlowChatTransportEvent>
    private let continuation: AsyncStream<iFlowChatTransportEvent>.Continuation

    init(
        historyResponses: [iFlowChatHistoryPayload],
        sessionsResponses: [iFlowChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<iFlowChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> iFlowChatHistoryPayload {
        ...
        return self.historyResponses.last ?? iFlowChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [iFlowChatAttachmentPayload]) async throws -> iFlowChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return iFlowChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> iFlowChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? iFlowChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: iFlowChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = iFlowChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = iFlowChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { iFlowChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = iFlowChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = iFlowChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = iFlowChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { iFlowChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
