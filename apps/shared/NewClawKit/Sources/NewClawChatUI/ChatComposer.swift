...
@MainActor
struct NewClawChatComposer: View {
    @Bindable var viewModel: NewClawChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(NewClawChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(NewClawChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \NewClawPendingAttachment.id)
                { (att: NewClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            NewClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(NewClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(NewClawChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(NewClawChatTheme.subtleCard)
        .clipShape(Capsule())
    }
