...
private let chatUILogger = Logger(subsystem: "ai.newclaw", category: "NewClawChatUI")

@MainActor
@Observable
public final class NewClawChatViewModel {
    public private(set) var messages: [NewClawChatMessage] = []
    ...
    public var attachments: [NewClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [NewClawChatPendingToolCall] = []
    public private(set) var sessions: [NewClawChatSessionEntry] = []
    private let transport: any NewClawChatTransport
    ...
    private var pendingToolCallsById: [String: NewClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any NewClawChatTransport) {
    ...
    public var sessionChoices: [NewClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [NewClawChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [NewClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: NewClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [NewClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: NewClawChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [NewClawChatMessage]) -> [NewClawChatMessage] {
        var result: [NewClawChatMessage] = []
    ...
    private static func dedupeKey(for message: NewClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [NewClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> NewClawChatAttachmentPayload in
            NewClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> NewClawChatSessionEntry {
        NewClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: NewClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: NewClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: NewClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = NewClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> NewClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
