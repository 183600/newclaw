...
private final class TestChatTransport: @unchecked Sendable, NewClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [NewClawChatHistoryPayload]
    private let sessionsResponses: [NewClawChatSessionsListResponse]

    private let stream: AsyncStream<NewClawChatTransportEvent>
    private let continuation: AsyncStream<NewClawChatTransportEvent>.Continuation

    init(
        historyResponses: [NewClawChatHistoryPayload],
        sessionsResponses: [NewClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<NewClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> NewClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? NewClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [NewClawChatAttachmentPayload]) async throws -> NewClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return NewClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> NewClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? NewClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: NewClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = NewClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = NewClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { NewClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = NewClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = NewClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = NewClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { NewClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
