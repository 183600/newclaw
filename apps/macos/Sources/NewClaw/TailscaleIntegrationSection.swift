...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.newclaw/newclaw.json. Prefer NEWCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.newclaw/newclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.newclaw/newclaw.json. Restart the gateway to apply."
        }
