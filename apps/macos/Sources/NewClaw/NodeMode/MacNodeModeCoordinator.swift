...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.newclaw", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: NewClawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [NewClawCapability.canvas.rawValue, NewClawCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(NewClawCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if NewClawLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(NewClawCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(NewClawCapability.camera.rawValue) {
        ...
        if capsSet.contains(NewClawCapability.location.rawValue) {
