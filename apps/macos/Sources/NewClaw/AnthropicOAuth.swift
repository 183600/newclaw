...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (NewClaw token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: NewClawOAuthStore.AnthropicOAuthStatus = NewClawOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.newclaw", category: "anthropic-oauth")
...
enum NewClawOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let newclawOAuthDirEnv = "NEWCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "NewClaw OAuth token file not found"
            case .unreadableFile: "NewClaw OAuth token file not readable"
            case .invalidJSON: "NewClaw OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in NewClaw OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "NewClaw OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.newclawOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".newclaw", isDirectory: true)
