...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.newclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> NewClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: NewClawSessionPreviewEntry,
    ...
    private static func cache(payload: NewClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: NewClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [NewClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [NewClawKit.AnyCodable]) -> [NewClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(NewClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: NewClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: NewClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: NewClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: NewClawChatMessage) -> String? {
