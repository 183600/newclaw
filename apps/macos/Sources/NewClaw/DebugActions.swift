...
enum DebugActions {
    private static let verboseDefaultsKey = "newclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "newclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = NewClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "NewClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = NewClawPaths.configURL
        guard
