...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["NEWCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/newclaw")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("newclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("newclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/newclaw/ (rolling or stdout), or nil if none exist.
