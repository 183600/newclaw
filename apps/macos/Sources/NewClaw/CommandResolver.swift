...
enum CommandResolver {
    private static let projectRootDefaultsKey = "newclaw.gatewayProjectRootPath"
    private static let helperName = "newclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let newclawEntry = root.appendingPathComponent("newclaw.mjs").path
        if FileManager().isReadableFile(atPath: newclawEntry) { return newclawEntry }
        let binEntry = root.appendingPathComponent("bin/newclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/newclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let newclawPaths = self.newclawManagedPaths(home: home)
        if !newclawPaths.isEmpty {
            extras.insert(contentsOf: newclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + newclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func newclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".newclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func newclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectNewClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("newclaw.mjs").path,
            root.appendingPathComponent("bin/newclaw.js").path,
        ]
    ...
    static func hasAnyNewClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.newclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func newclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let newclawPath = self.projectNewClawExecutable(projectRoot: root) {
                return [newclawPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "newclaw", subcommand] + extraArgs
            }
            if let newclawPath = self.newclawExecutable(searchPaths: searchPaths) {
                return [newclawPath, subcommand] + extraArgs
            }

            let missingEntry = """
            newclaw entrypoint missing (looked for dist/index.js or newclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real newclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/newclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v newclaw >/dev/null 2>&1; then
          CLI="$(command -v newclaw)"
          newclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/newclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/newclaw.mjs"
            node "$PRJ/newclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/newclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/newclaw.js"
            node "$PRJ/bin/newclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent newclaw"
          pnpm --silent newclaw \(quotedArgs);
        else
          echo "newclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? NewClawConfigFile.loadDict()
