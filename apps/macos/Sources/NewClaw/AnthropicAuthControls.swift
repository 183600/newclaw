...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: NewClawOAuthStore.AnthropicOAuthStatus = NewClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: NewClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = NewClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = NewClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try NewClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. NewClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: NewClawOAuthStore.AnthropicOAuthStatus,
