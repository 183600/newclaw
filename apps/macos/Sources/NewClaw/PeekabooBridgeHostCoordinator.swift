...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.newclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: NewClawPeekabooBridgeServices?
    private static var newclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("NewClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = NewClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.newclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.newclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class NewClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.newclaw.peekaboo")
