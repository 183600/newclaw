...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: NewClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.newclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: NewClawKit.AnyCodable]()) {
                $0[$1.key] = NewClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: NewClawProtocol.AnyCodable?) -> [String: NewClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: NewClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: NewClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: NewClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("newclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("newclaw.control.agent")
}
