...
private let webChatSwiftLogger = Logger(subsystem: "ai.newclaw", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: NewClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> NewClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(NewClawChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> NewClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                NewClawGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<NewClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any NewClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = NewClawChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: NewClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<NewClawChatView>) -> NSViewController
    {
