...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.iflow", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in iFlowBonjour.gatewayServiceDomains {
            ...
            browser.start(queue: DispatchQueue(label: "ai.iflow.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = iFlowBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = iFlowBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = iFlowBonjour.wideAreaGatewayServiceDomain else { return false }
