...
private let webChatSwiftLogger = Logger(subsystem: "ai.iflow", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: iFlowChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> iFlowChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(iFlowChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> iFlowChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                iFlowGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<iFlowChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any iFlowChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = iFlowChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: iFlowChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<iFlowChatView>) -> NSViewController
    {
