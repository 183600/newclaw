...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (iFlow token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: iFlowOAuthStore.AnthropicOAuthStatus = iFlowOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.iflow", category: "anthropic-oauth")
...
enum iFlowOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let iflowOAuthDirEnv = "IFLOW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "iFlow OAuth token file not found"
            case .unreadableFile: "iFlow OAuth token file not readable"
            case .invalidJSON: "iFlow OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in iFlow OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "iFlow OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.iflowOAuthDirEnv]?
