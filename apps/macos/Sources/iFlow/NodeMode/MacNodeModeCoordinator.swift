...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.iflow", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: iFlowNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [iFlowCapability.canvas.rawValue, iFlowCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(iFlowCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if iFlowLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(iFlowCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(iFlowCapability.camera.rawValue) {
        ...
        if capsSet.contains(iFlowCapability.location.rawValue) {
