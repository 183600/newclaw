...
enum CommandResolver {
    private static let projectRootDefaultsKey = "iflow.gatewayProjectRootPath"
    private static let helperName = "iflow"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let iflowEntry = root.appendingPathComponent("iflow.mjs").path
        if FileManager().isReadableFile(atPath: iflowEntry) { return iflowEntry }
        let binEntry = root.appendingPathComponent("bin/iflow.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/iflow")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let iflowPaths = self.iflowManagedPaths(home: home)
        if !iflowPaths.isEmpty {
            extras.insert(contentsOf: iflowPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + iflowPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func iflowManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".iflow"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func iflowExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectiFlowExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("iflow.mjs").path,
            root.appendingPathComponent("bin/iflow.js").path,
        ]
    ...
    static func hasAnyiFlowInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.iflowExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let iflowPath = self.projectiFlowExecutable(projectRoot: root) {
                return [iflowPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "iflow", subcommand] + extraArgs
            }
            if let iflowPath = self.iflowExecutable(searchPaths: searchPaths) {
                return [iflowPath, subcommand] + extraArgs
            }

            let missingEntry = """
            iflow entrypoint missing (looked for dist/index.js or iflow.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real iflow CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/iflow"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/iflow.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/iflow.mjs"
            node "$PRJ/iflow.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/iflow.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/iflow.js"
            node "$PRJ/bin/iflow.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent iflow"
          pnpm --silent iflow \(quotedArgs);
        else
          echo "iflow CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? iFlowConfigFile.loadDict()
