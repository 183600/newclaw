...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["IFLOW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/iflow")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("iflow-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("iflow-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/iflow/ (rolling or stdout), or nil if none exist.
