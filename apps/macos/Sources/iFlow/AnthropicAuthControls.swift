...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: iFlowOAuthStore.AnthropicOAuthStatus = iFlowOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: iFlowOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = iFlowOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = iFlowOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try iFlowOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. iFlow can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: iFlowOAuthStore.AnthropicOAuthStatus,
