...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.iflow/iflow.json. Prefer IFLOW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.iflow/iflow.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.iflow/iflow.json. Restart the gateway to apply."
        }
