...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: iFlowProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.iflow", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: iFlowKit.AnyCodable]()) {
                $0[$1.key] = iFlowKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: iFlowProtocol.AnyCodable?) -> [String: iFlowProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: iFlowProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: iFlowKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: iFlowProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("iflow.control.heartbeat")
    static let controlAgentEvent = Notification.Name("iflow.control.agent")
}
