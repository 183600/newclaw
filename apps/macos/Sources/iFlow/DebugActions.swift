...
enum DebugActions {
    private static let verboseDefaultsKey = "iflow.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "iflow.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = iFlowPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "iFlow", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = iFlowPaths.configURL
        guard
