...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.iflow", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: iFlowPeekabooBridgeServices?
    private static var iflowSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("iFlow", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = iFlowPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.iflowSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.iflowSocketPath, privacy: .public)")
    }
...
@MainActor
private final class iFlowPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.iflow.peekaboo")
