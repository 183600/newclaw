...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.iflow", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> iFlowSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: iFlowSessionPreviewEntry,
    ...
    private static func cache(payload: iFlowSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: iFlowChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [iFlowKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [iFlowKit.AnyCodable]) -> [iFlowChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(iFlowChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: iFlowChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: iFlowChatMessage) -> Bool {
    ...
    private static func toolNames(for message: iFlowChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: iFlowChatMessage) -> String? {
