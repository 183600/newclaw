...
enum iFlowLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = iFlowOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = iFlowFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        iFlowLogging.bootstrapIfNeeded()
        let label = iFlowLogging.makeLabel(subsystem: subsystem, category: category)
...
struct iFlowOSLogHandler: LogHandler {
...
struct iFlowFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = iFlowLogging.parseLabel(self.label)
