...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.newclaw", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in NewClawBonjour.gatewayServiceDomains {
            ...
            browser.start(queue: DispatchQueue(label: "ai.newclaw.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = NewClawBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = NewClawBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = NewClawBonjour.wideAreaGatewayServiceDomain else { return false }
