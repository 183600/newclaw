...
struct NewClawMacCLI {
    static func main() async {
        ...
        default:
            fputs("newclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      newclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      newclaw-mac discover [--timeout <ms>] [--json] [--include-local]
      newclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      newclaw-mac connect
      newclaw-mac connect --url ws://127.0.0.1:18789 --json
      newclaw-mac discover --timeout 3000 --json
      newclaw-mac wizard --mode local
    """)
}
