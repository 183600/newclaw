...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: iFlowProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: iFlowProtocol.AnyCodable] = [
            "presence": .init([iFlowProtocol.AnyCodable(entry)]),
        ]
        let payload = iFlowProtocol.AnyCodable(payloadMap)
