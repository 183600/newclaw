...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: NewClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: NewClawProtocol.AnyCodable] = [
            "presence": .init([NewClawProtocol.AnyCodable(entry)]),
        ]
        let payload = NewClawProtocol.AnyCodable(payloadMap)
