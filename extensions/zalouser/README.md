...
Claw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
claw channels login --channel zalouser
...
```bash
claw message send --channel zalouser --target <threadId> --message "Hello from Claw!"
```
...
```bash
claw channels login --channel zalouser              # Login via QR
claw channels login --channel zalouser --account work
claw channels status --probe
claw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
claw directory self --channel zalouser
claw directory peers list --channel zalouser --query "name"
claw directory groups list --channel zalouser --query "work"
claw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
claw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
claw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `claw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
claw channels login --channel zalouser --account work
claw message send --channel zalouser --account work --target <id> --message "Hello"
