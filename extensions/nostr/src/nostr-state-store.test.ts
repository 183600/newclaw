...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "iflow-nostr-"));
  process.env.IFLOW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.IFLOW_STATE_DIR?.trim() || env.IFLOW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".iflow");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.IFLOW_STATE_DIR = previous;
    }
