...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.NEWCLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "newclaw-nostr-"));
  process.env.NEWCLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.NEWCLAW_STATE_DIR?.trim() || env.NEWCLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".newclaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.NEWCLAW_STATE_DIR = previous;
    }
